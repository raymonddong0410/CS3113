...
void LevelB::initialise()
{
   ...
   */
   ...
   mGameState.mc = new Entity(
      {mOrigin.x - 950.0f, mOrigin.y - 200.0f}, // position
      {100.0f, 100.0f},             // scale
   ...
   mGameState.mc->setColliderDimensions({
      mGameState.mc->getScale().x / 2.0f,
      mGameState.mc->getScale().y / 1.1f
   });
   ...
   */
   ...
   mGameState.enemy = new Entity(
      {mOrigin.x - 700.0f, mOrigin.y - 100.0f}, // position
      {100.0f, 100.0f},             // scale
      "assets/enemy1.png",                   // texture file address
      ATLAS,                                    // single image or atlas?
      { 4, 4 },                                 // atlas dimensions
   ...
   mGameState.enemy->setColliderDimensions({
      mGameState.enemy->getScale().x / 2.0f,
      mGameState.enemy->getScale().y / 1.1f
   });

   // second enemy
   mGameState.enemy2 = new Entity(
      {mOrigin.x - 325.0f, mOrigin.y - 100.0f}, // position
      {100.0f, 100.0f},             // scale
      "assets/enemy3.png",                   // texture file address
      ATLAS,                                    // single image or atlas?
      { 4, 4 },                                 // atlas dimensions
      enemyAnimationAtlas,                    // actual atlas
      NPC                                    // entity type
   ...
   mGameState.enemy2->setColliderDimensions({
      mGameState.enemy2->getScale().x / 2.0f,
      mGameState.enemy2->getScale().y / 1.1f
   });
