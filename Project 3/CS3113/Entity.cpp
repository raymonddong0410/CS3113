...
Entity::Entity() : mPosition {0.0f, 0.0f}, mMovement {0.0f, 0.0f}, 
                   ...
                   mSpriteSheetDimensions {}, mEntityMovement {IDLE}, 
                   mAnimationAtlas {{}}, mAnimationIndices {}, mFrameSpeed {0},
                   mEntityType {NONE}, mGameStatus {PLAYING} { }

Entity::Entity(Vector2 position, Vector2 scale, const char *textureFilepath,
    EntityType EntityType) : mPosition {position}, mVelocity {0.0f, 0.0f}, mAcceleration {0.0f, 0.0f}, 
    mScale {scale}, mMovement {0.0f, 0.0f}, mColliderDimensions {scale},
    mTexture {LoadTexture(textureFilepath)}, mTextureType {SINGLE}, 
    mEntityMovement {IDLE}, mAnimationAtlas {{}}, mAnimationIndices {}, 
    mFrameSpeed {0}, mSpeed {DEFAULT_SPEED}, mAngle {0.0f},
    mEntityType {EntityType}, mGameStatus {PLAYING} { }

Entity::Entity(Vector2 position, Vector2 scale, const char *textureFilepath, 
        TextureType textureType, Vector2 spriteSheetDimensions, std::map<EntityMovement, 
        ...
        mAnimationAtlas {animationAtlas}, mEntityMovement {IDLE}, 
        mAnimationIndices {animationAtlas.at(IDLE)}, 
        mFrameSpeed {DEFAULT_FRAME_SPEED}, mAngle { 0.0f }, 
        mSpeed { DEFAULT_SPEED }, mEntityType {EntityType}, mGameStatus {PLAYING} { }
    ...
    for (int i = 0; i < collisionCheckCount; i++)
    {
        ...
        if (isColliding(collidableEntity))
        {
            ...
            if (mVelocity.y > 0) 
            {
                ...
                // if bottom of ship collides with platform --> win
                // if bottom of ship collides with something else --> lose
                if (collidableEntity->mEntityType == PLATFORM) {
                    mGameStatus = WIN;
                }
                else {
                    mGameStatus = LOSE;
...
{
    mAnimationIndices = mAnimationAtlas.at(mEntityMovement);
...
Entity *platforms, int platformCount)
{
    ...
    // comment out so movement only based on acceleration
    ...
    // for platforms that indicate a win
    ...
    if (mTextureType == ATLAS && GetLength(mAcceleration) != 0) 
        animate(deltaTime);

    if (mTextureType == ATLAS && mEntityType == FLAME) {
        animate(deltaTime);
    }
}
