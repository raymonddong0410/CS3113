...
Entity::Entity(Vector2 position, Vector2 scale, const char *textureFilepath, 
    ...
    mAttackTexture {LoadTexture(attackTextureFilepath)},
...
Entity::~Entity() { 
    ...
    // extra line for deconstructor to unload texture if attack atlas is present for entity
    if (mAttackTexture.id != 0) UnloadTexture(mAttackTexture);
     };

void Entity::attack() {
    if (mIsAttacking || mEntityType != PLAYER) return;

    mIsAttacking = true;
    mAttackTimer = ATTACK_DURATION;

    // set to correct direction (check attack direction in case repeated call)
    if (mDirection == LEFT || mDirection == ATTACK_LEFT) {
        setDirection(ATTACK_LEFT);
    } 
    else if (mDirection == RIGHT || mDirection == ATTACK_RIGHT) {
        setDirection(ATTACK_RIGHT);
    }
    else if (mDirection == UP || mDirection == ATTACK_UP) {
        setDirection(ATTACK_UP);
    }
    else if (mDirection == DOWN || mDirection == ATTACK_DOWN) {
        setDirection(ATTACK_DOWN);
    }
}
...
void Entity::checkCollisionY(Entity **collidableEntities, int collisionCheckCount)
{
    for (int i = 0; i < collisionCheckCount; i++)
    {
        ...
        if (isColliding(collidableEntity))
        {
            ...
            // check if colliding with npc or goal to decide status of level
            if (collidableEntity->mEntityType == GOAL) {
                mIsCollidingGoal = true;
            }
            // add attacking logic for npcs
            if (this->mEntityType == PLAYER && collidableEntity->getEntityType() == NPC)
            {
                if (mIsAttacking)
                {
                    collidableEntity->deactivate();
...
void Entity::AIWander() { // change logic so go back and forth
    if (mIsCollidingLeft) {
        mDirection = RIGHT;
...
void Entity::AIFollow(Entity *target)
{
    ...
    {
    case IDLE:
        if (Vector2Distance(mPosition, target->getPosition()) < 180.0f) 
            mAIState = FOLLOWING;
        break;
...
void Entity::AIOrbit(float deltaTime) {
    mAngle += ORBIT_SPEED * deltaTime;
    mPosition.x = mCenter.x + mRadius * cos(mAngle);
    mPosition.y = mCenter.y + mRadius * sin(mAngle);
}
    ...
    Entity **collidableEntities, int collisionCheckCount)
{
    ...
    if (mIsAttacking) {
        // chracter not moving if attacking
        mMovement = {0.0f, 0.0f};

        mAttackTimer -= deltaTime;
        if (mAttackTimer <= 0.0f) {
            mIsAttacking = false;
            // reset direction back to normal direction
            if (mDirection == ATTACK_LEFT) setDirection(LEFT);
            else if (mDirection == ATTACK_RIGHT) setDirection(RIGHT);
            else if (mDirection == ATTACK_UP) setDirection(UP);
            else if (mDirection == ATTACK_DOWN) setDirection(DOWN);
        }
    }
    ...
    // update animation logic so that the attack and movement is animated
    if (mTextureType == ATLAS)
    {
        ...
        else if (GetLength(mMovement) != 0)
        {
            // if (mEntityType == PLAYER) {
            //     if (mIsCollidingBottom) animate(deltaTime);
            // } else {
            //     animate(deltaTime);
            // }
            animate(deltaTime);
...
void Entity::render()
{
    ...
    // if attacking switch texture
    Texture2D *activeTexture = &mTexture;
    if (mIsAttacking) {
        activeTexture = &mAttackTexture;
    }
    ...
    switch (mTextureType)
    {
        case SINGLE:
            ...
            textureArea = {
                ...
                static_cast<float>(activeTexture->width),
                static_cast<float>(activeTexture->height)
            };
            break;
        case ATLAS:
